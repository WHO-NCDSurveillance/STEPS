...
j=NULL
k=NULL
for (i in unique(indicator_matrix$section)[1])
{
  data = analysis_data
  ...
  for(j in unique(section_matrix$indicator_short_desc))
  {
    ...
    for(sub_row in 1:nrow(pre_sub_matrix))
    {
      ...
      similarity_ind = unique(gsub("\\d{1,2}$", "", subset_indicators))
  ...
  #title_text = ftext(translated_text, color='green') %>% fpar()
  # Create a text formatting object with green color
  green_text_format <- fp_text(color = "#008000", font.size = 16, underlined = F, font.family = 'Source Sans Pro')  # Specify font size
  # Create a formatted paragraph with the translated text
  title_text <- fpar(ftext(translated_text, prop = green_text_format))
  docx = read_docx('templates/edited_styles.docx')%>% body_add_fpar(title_text, style = "heading 1", pos='on')
...
databook <- read_docx('templates/edited_styles.docx')###path = "templates/style_template.docx"
...
####Adding ToC
databook = databook %>% body_add_toc()
#databook = databook %>% update_toc()
