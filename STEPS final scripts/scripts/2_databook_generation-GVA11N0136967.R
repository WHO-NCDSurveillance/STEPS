...
for (i in unique(indicator_matrix$section))
{
  ...
  title_text = ftext(translated_text,fp_text(color = "#008000", bold = F, font.size = 14, font.family = 'Source Sans Pro')) %>% fpar()
  docx = read_docx()%>% 
         body_add(title_text, style = "Normal")
...
for(i in 1:(1+length(unique(indicator_matrix$section)))){
  ...
  databook = body_add_docx(databook, path, pos = "on") 
...
####
###
# print(databook, target =paste0('outputs/Databook.docx'))
