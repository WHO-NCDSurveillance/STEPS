##Reading in data
data = read_excel('data_input/data.xlsx')# Reads data from an Excel file located at 'data_input/data.xlsx' into a data frame called 'data'.
...
for (i in extracted_variables_77_88)
{
  ...
  } else{}# If no matches are found, do nothing.
}
...
for(i in paste0('restric_',unique_NA_numbers,'_vars'))
{
  ...
  } else{}# If no variables are found, do nothing.
}
...
# Setting NA to binary indicators when primary variables are missing 

#primary_vars_with_logexp = do.call('c',strsplit(matrix_with_logexp$primary_variables, "[;]")) 
ind = NULL
for(ind in 1:nrow(matrix_with_logexp))
{
  sub_ind = tolower(do.call('c',strsplit(matrix_with_logexp[ind,]$indicator_var, "[;]")))  
  sub_primary_vars = tolower(do.call('c',strsplit(matrix_with_logexp[ind,]$primary_variables, "[;]"))) 
  sub = NULL
  for(sub in sub_ind)
  {
    eval(parse(text=paste0('data$',sub,'[',paste0('is.na(data$',sub_primary_vars,')', collapse = '|'),']=NA')))
  }
}
...
indicator_matrix = indicator_matrix %>% rowwise %>%
  mutate(
    # Identify missing primary variables in 'data' for each variable in 'indicator_var'    
         var_missing = list(eval(parse(text=paste0('c(',paste0('all(is.na(data$',do.call('c',strsplit(indicator_var, "[;]")),'))', collapse = ','),')')))),
         ### Clean and transform the 'pop_subset' string for easier parsing         
         rev_popsubset = gsub('^all$','TRUE',pop_subset),
         rev_popsubset = gsub(';all',';TRUE',rev_popsubset),
         rev_popsubset = gsub('all;','TRUE;',rev_popsubset),
    # Evaluate missing population subsets based on 'data' and 'pop_subset'
         pop_missing = list(eval(parse(text=paste0('c(',paste0('with(data, !any(',do.call('c',strsplit(rev_popsubset, "[;]")),',na.rm=T))', collapse = ','),')')))),
         #
    #Combine the missing variable and population checks
         rev_var_missing = list(eval(parse(text=paste0('c(',paste0(unlist(var_missing),'|',unlist(pop_missing), collapse = ','),')')))),
         ###Identify positions of TRUE (missing) conditions
         pos_true = paste0(as.character(list(grep('TRUE',rev_var_missing))), collapse = ','),# Get positions of 'TRUE' values
         pos_true = ifelse(pos_true=='integer(0)'|excl_missing_ind == 'no','999',pos_true),# Handle cases where no 'TRUE' is found
         ###updated n_semicolons-Adjust the count of semicolons based on the removal of missing indicators
...
indicator_matrix = indicator_matrix %>% rowwise %>% mutate(
  # Update 'indicator_var' based on the number of variables. If there's only one variable, it remains unchanged; 
  # otherwise, multiple variables are concatenated using a semicolon.
  ...
  ####logic_condition_var
  #### Process 'logic_condition_var' similarly to 'indicator_var'
  ...
  # If there's only one logical condition variable, it remains unchanged;
  # otherwise, multiple conditions are concatenated using a semicolon.
  ...
  ####pop_subset
  #### Process 'pop_subset' similarly to 'logic_condition_var'
  ...
  # If there's only one population subset, it remains unchanged;
  # otherwise, multiple subsets are concatenated using a semicolon.
  pop_subset = list(if(len_ind_var==1 ){as.character(pop_subset)}else{paste0(eval(parse(text = as.character(pop_subset))), collapse = ';')}),
  #####subtitle1
  ##### Process 'subtitle1' based on the number of variables
  ...
  subtitle1 = list(if(len_ind_var==1 & is.na(subtitle2)){as.character(subtitle1)}
                     ...
                     # Retain 'subtitle1' if 'subtitle2' is not missing
                     subtitle1
                   }else{paste0(eval(parse(text = as.character(subtitle1))), collapse = ';')}),
  # ###subtitle2
  ##### Process 'subtitle2' similarly to 'subtitle1'
  ...
  # #####type
  ##### Process 'type' similarly to other variables
  ...
  ####Variable types
  #### Reformat the processed variables for consistency
...
# Determine if the ISO code exists for the country
...
# Report on missing grouped variables
...
# Report on variables present in the xls form but missing from the dataset
...
# Report on variables with values assigned as '77' or '88' (for 'Don't know' or 'Refusals')
...
# Report on secondary variables that could not be derived due to missing primary variables
...
# Report on the status of the script execution based on ISO code existence
...
# Compile all processing steps into a summary report
...
#########################
# Create a flex table to present the report
...
# Insert the processing report into a Word document
...
# Add the processing report table to the document at a specific location
