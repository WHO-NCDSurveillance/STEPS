...
##Checking weight steps in matrix match what is in the dataset
wt_steps_matrix = unique(indicator_matrix$weight_step)
wt_steps_dataset = grep('wstep', names(data), value = T)

# Identify matrix variables missing from the dataset
missing_wt_steps = wt_steps_matrix[
  !wt_steps_matrix %in% wt_steps_dataset
]

# Report results
if (length(missing_wt_steps) > 0) {
  
  stop(
    paste0(
      "ERROR: The following weight_step variables are present in ",
      "indicator_matrix but missing from the dataset: ",
      paste(missing_wt_steps, collapse = ", ")
    )
  )
  ...
  message(
    paste0(
      ...
      length(wt_steps_matrix),
      " weight_step variables in indicator_matrix are present in the dataset."
    )
  )
}
...
outofrange_logic = reduced_xml %>%
  ...
  dplyr::filter(select_fn == FALSE & name!='dob') # Process constraint expressions
#
...
#### Indicator and secondary variable matrix ####

#### Check if primary variables specificed in the matrix exist in the dataset ####
