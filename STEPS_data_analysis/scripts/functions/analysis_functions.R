#####
# Define a function to generate a summary table for numeric Demographics data
...
#####
# Define a function to generate a summary table for categorical Demographics data
...
est_ciprop = svyby(formula, by = ~eval(parse(text=col_strat))+ eval(parse(text=row_strat)), design = svy_datum, FUN = svymean, method = "lo", df = degrees_freedom, vartype = 'ci', na.rm = T)%>%
            full_join(svyby(formula, by = ~eval(parse(text=row_strat)), design = svy_datum, FUN = svymean, method = "lo", df = degrees_freedom, vartype = 'ci', na.rm = T) %>%
                        mutate(`eval(parse(text = col_strat))` = 'Total'))%>%mutate(ci_l = ifelse(ci_l<0,0,ci_l))
##Totals
# Calculate total weighted percentages
colstrat_var_est_ciprop = svyby(formula, by = ~eval(parse(text=col_strat)), design = svy_datum, FUN = svymean, method = "lo", df = degrees_freedom, vartype = 'ci', na.rm = T)%>%
